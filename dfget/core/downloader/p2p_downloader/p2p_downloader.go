	...
	for {
		...
		if err != nil {
			...
			if code == constants.CodePeerContinue {
				...
				p2p.finishTask(response, ctx, pieceWriter)
				return nil
			} else {
...
func (p2p *P2PDownloader) finishTask(response *types.PullPieceTaskResponse, ctx context.Context, pieceWriter PieceWriter) {
	...
	err := pieceWriter.PostRun(response, ctx)
	if err != nil {
